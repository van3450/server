...
const menuCamPos = [-1828.8, -870.1, 3.1];//-1828.8 -867.6
const menuCamDist = 2.5;
const playerPosZ = [3.151, 3.155, 3.234];
const cos30 = 0.866;
const sin30 = 0.5;
...
mp.events.add('playerJoined.client', (characters) => {
    currentCharacter = 0;
    if (characters != null) {
...
mp.events.add('charChoose.client', (charnumber, place) => {     //place = 0 - последняя точка входа, 1 - дом, 2 - госка
    if(isBinding){
        binding(false);
        isBinding = false;
    }
    mp.events.callRemote('charChoose.server', charnumber, place);
});

mp.events.add('charChooseAns.client', (ans) => {     //0 - не успешно     1 - успешно
    if (ans == 0) {
        if(!isBinding){
            binding(true);
            isBinding = true;
...
mp.events.add('chooseRight.client', () => {
    chooseRight();
});
mp.events.add('chooseLeft.client', () => {
    chooseLeft();
});
...
let choose = function() {
    //ui.callCEF('choose');
    //temp
    mp.events.call('charChoose.client', currentCharacter, 0);
};
